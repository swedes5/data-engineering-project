...
def load_benzene_data(nrows):
    benzene_data = pd.read_csv('https://docs.google.com/spreadsheets/d/e/2PACX-1vR0L2ZCt6N3E_tOhbYF1zPfam0J8p72QVVVMH_8obUfSqLJDLyBRZ5lv5JVOexw5v7_4qFTUu16wdRb/pub?output=csv', nrows=nrows)
    #'benzene copy.csv'
...
def load_co_data(nrows):
    co_data = pd.read_csv('https://docs.google.com/spreadsheets/d/e/2PACX-1vQZrvmVJbWwcBV9Ha2Nl_eZLjcBC4bTI9aLrRp0OMTIoZyTWEXESheOdLDBHh3vNICEled6fbRWHSI8/pub?output=csv', nrows=nrows)
    #'carbon_monoxide copy.csv'
...
def load_no2_data(nrows):
    no2_data = pd.read_csv('https://docs.google.com/spreadsheets/d/e/2PACX-1vQioXuHCtE6Oe5pUJyzN_BK_mKDeXk8cJzdNE0SZktl9XOtziXruNY8IGWAkKb3nqJrbE7-alhRmIQw/pub?output=csv', nrows=nrows)
    #'nitrogen_dioxide copy.csv'
...
def load_so2_data(nrows):
    so2_data = pd.read_csv('https://docs.google.com/spreadsheets/d/e/2PACX-1vSrDAYH4Kel4WgVzoBSUsq3HLt0SfV-VmGTzH9O3gBQkc0lWlQ4aMZdqgHoTgK78SYrHKS7e4APkm75/pub?output=csv', nrows=nrows)
    #'sulfer_dioxide copy.csv'
...
benzene_data = load_benzene_data(650000) #loading all the rows
co_data = load_co_data(650000)
no2_data = load_no2_data(650000)
so2_data = load_so2_data(650000)
